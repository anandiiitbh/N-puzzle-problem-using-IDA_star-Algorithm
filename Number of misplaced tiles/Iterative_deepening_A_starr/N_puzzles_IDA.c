...
int print_solution(const int **start, int N, const char *desc){
	...
	while (1) {
		...
		move = desc[i];
		if (move == '\0'){
			printf("NUMBER OF STEPS REQUIRED TO REACH GOAL STATE: %d\n",count);
			printf("\nGOT IT...........:):)\n");
			for (i = 0; i < N; i++){
				free(board[i]);
			}
...
int comp( int *current_rows, int *current_cols, const int *goal_rows, const int * goal_cols,int N_squared){
	int count=0;
	for(int i=1;i<N_squared;i++){
		if(current_rows[i]==goal_rows[i] && current_cols[i]==goal_cols[i])
			continue;
		else
			count++;
	}
...
int run(int **start, const int **goal, int N){
	...
	desc[1] = '\0';
	h_score = 0;
	h_score=comp(current_rows, current_cols, (const int *)goal_rows, (const int *) goal_cols, N_squared);
	threshold = h_score;	
	while (1){			
